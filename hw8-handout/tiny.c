...
void serve_static(int fd, char *filename, int filesize, int size_flag, rangeNode *node);
 ...
 */
/* $begin serve_static */
void serve_static(int fd, char *filename, int filesize, int size_flag, rangeNode *range)
{
  int srcfd;
  ...
  memset(buf,0,MAXBUF);
  //When type 1, second must greater than first and they both need to in the filesize range
  if (range->type == 1){
    if ((range->first >= range->second) || ((range->first) < 0) || (range->second > (filesize-1))){
      sprintf(buf, "%sHTTP/1.1 416 Range Not Satisfiable\r\n", buf);
    }
    else{
      length = range->second - range->first + 1;
      sprintf(buf, "%sHTTP/1.1 206 Partial Content\r\n",buf);
    }
  }

  //When type 2, second is always filesize-1
  else if (range->type == 2){

    range->second = filesize - 1;

    if ((range->first < 0) || (range->first > filesize - 2) ){
      sprintf(buf, "%sHTTP/1.1 416 Range Not Satisfiable\r\n", buf);
    }
    else{
      length = range->second - range->first + 1;
  ...
  //When type 3, first is filesize+range->first and second is filesize-1
  else if (range->type == 3){
    range->first = filesize + range->first;
    range->second = filesize - 1;
  ...
  if (rio_writen(fd, srcp+(range->first), length) < length) {
